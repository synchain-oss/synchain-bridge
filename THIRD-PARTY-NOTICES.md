...
- **macOS 构建的第三方闭包**(与 Windows 不同,按平台核算):
  ...
  - **zlib**:mac 侧 `USE_ZLIB` 保持上游默认 ON(与 vcpkg port 一致,permessage-deflate 行为跨平台不漂移),
    但链接的是 **macOS SDK 自带的系统 libz**,不随本仓库分发 —— 与上面 WebView2 Runtime 同一处理:
    平台系统库不进第三方声明闭包(U2 的 GPLv3 系统库例外默认解释)。上表的 zlib 条目只覆盖 Windows 侧
    vcpkg 静态链接的那一份。
  - WKWebView / AudioUnit 等 Apple 框架同属 macOS 系统库,不进闭包、不随分发。
