...
//   Chromium 必须照常记 paint 记录」。本机真 WebView2 宿主(pluginval)实测**三批 11 次全有记录**
//   (N ≈ 710~780 ms,不是保险路的 ≈2500),本机上成立;但那不是用户的 DAW,这条判据照留。
//
// 【改前的时序:本机已量,不再是推断】让旧触发链(DOMContentLoaded + 两层 rAF)与 paint 记录
//   **两条都跑、都只记时刻**,送两者之差 —— pluginval 宿主上 `oldTrigger − firstPaint`
//   **12 次全为负,−33 ~ −52 ms(中位 −46,非负 0 次)**:改前那条信号确实早于 first-paint,
//   揭窗时页面一帧未画 ⇒ 本仓的成因解释成立。同台对照:**修后共三批 11 次全为正,
//   区间 +14~18 ms**(基线 5 次 +15..+18 / 兼容性那批的复原复测 3 次 +17/+14/+17 /
//   本次测量后的复原复测 3 次 +18/+17/+18)。⚠ **三批各带自己的 n,别合并成一个「×3」,
//   也别把某一批的 n 配上合计的区间** —— 本卡在这个记账上连栽四次,逐批列在 CHANGELOG.md 的
//   SL-433 词条里,改任何一处都回去对一遍。
//   符号翻正且幅度对得上(均值 −44 → 中位 +17,差 ≈60 ms 正是「等 paint 记录到达」补上的那一段)。
//   ⚠ 仪器必须**两条链都记**:只等 DCL 的那种测不出正值(信号发出时还没有 paint 记录,
//   只会打 `(no paint record)`),只等 paint 的那种测不出负值 —— 各有一个方向的盲区。
//   「无静默缺样本」的证据是那 12 次里 `(no paint record)` 与 `(paint delta unreadable)` 各 0 次。
//   ⚠ 这是 **pluginval 宿主**的读数,**不是用户的 DAW**;用户机是否同量级未测,
//   所以「白消不消失」仍以真机回验为准 —— 别把这组数读成「已经证明能修好」。
//
// 【真机回验的症状清单】(白之外还要看的两项,都没有静态判据接得住)
//   · 放行原因与条数比:开 N 次窗应有 N 行 first-frame signal。
//     ⚠ **别写成「出现 timeout 即异常」**(本卡第 3 轮复审订正:那句与本文件上面那条保险路
//     例外自相矛盾)—— 页内保险(约 2.5s)跑输这里的 3s 兜底时,放行原因是**合法地**掉成
//     timeout 的。分诊看两处:同一行的 `navFinished seen|not seen`,以及首帧信号那一行的
//     `after N ms` 量级(≈2500 ⇒ 保险路,见上面的回滚判据);
//   · **放行瞬间卡片还在 0.6s vbUp 淡入的头几帧** —— 插件路径关入场动画靠 boot() 早于放行,
//     而 [SL-433] 之后那是**竞态不是构造**(理由见 web/index.html 的 layoutForMode 处注释)。
//     命中形态是观感回归(浅占位 → 深 root → 浅卡片的明暗跳变),不是白;本机未量。
//     ⚠ **已知可以更强,本轮有意没收**(第 4 轮复审【建议】,转 SL-439):上面那 12 组数其实
//     推得出一个界 —— `oldTrigger = DCL + 两层 rAF`、两层 rAF 耗时 ≥ 0 ⇒
//     `DCL − firstPaint ≤ −33 ms`,而模块求值在 DCL 派发之前 ⇒ `boot() − firstPaint ≤ −33 ms`,
//     即这个宿主上动画至少在 first-paint 前 33 ms 就关掉了、本项不该命中。没收的理由:
//     那是「能更强」不是「写错了」,且它多挂两个前提(rAF 耗时非负、仪器确实取自 DCL 链);
//     更值的做法是在探针里再记一个 `boot()` 时刻,把这一格从「可观测项」变成「已量」。
//
...
// `PerformanceObserver` 的 paint 回调**结构上不可能**跑在 paint 记录存在之前(记录先入队,
