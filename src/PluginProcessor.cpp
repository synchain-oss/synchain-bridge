...
SynchainBridgeAudioProcessor::makeLayout() {
...
SynchainBridgeAudioProcessor::SynchainBridgeAudioProcessor()
  ...
  mBridgeServer.callbacks.onHandshake =
    [this](const juce::String& projectId,
      ...
      mBridgeServer.sendStatus(true, "Synchain Bridge",
  ...
  mBridgeServer.callbacks.onVolumeChange =
    [this](int pct) { requestWebVolume(pct); };
...
void SynchainBridgeAudioProcessor::prepareToPlay(double sampleRate,
                                                int samplesPerBlock) {
...
void SynchainBridgeAudioProcessor::processBlock(
  ...
  if (!mBridgeServer.isRunning()) return; // standby: no metering, no send
...
void SynchainBridgeAudioProcessor::getStateInformation(
...
void SynchainBridgeAudioProcessor::setStateInformation(
  ...
  if (auto xml = getXmlFromBinary(data, sizeInBytes)) {
    ...
    if (tree.isValid() && tree.hasType(mApvts.state.getType())) {
      ...
      setUiScale(static_cast<float>(
        static_cast<double>(tree.getProperty("uiScale", synchain::plugin::DefaultUiScale))));
    }
