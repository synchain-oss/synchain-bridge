...
class SynchainBridgeAudioProcessor : public juce::AudioProcessor,
                                     private juce::Timer {
public:
  ...
  int currentSampleRate() const {
    return static_cast<int>(mCurrentSampleRate.load(std::memory_order_relaxed));
  }
  ...
  void setUiScale(float s) {
    mUiScale = juce::jlimit(plugin::MinUiScale, plugin::MaxUiScale, s);
  }
  // 把当前 uiScale 写入全局设置文件（message 线程调用；见 WebViewEditor::handleSetUiScale）
  void persistUiScaleAsDefault();

  int getVolumePct() const {
    return mGainParam != nullptr
             ? juce::roundToInt(mGainParam->load(std::memory_order_relaxed) * 100.0f)
             : 100;
  }
  ...
  void requestWebVolume(int pct) {
    mPendingWebVolume.store(juce::jlimit(0, 200, pct), std::memory_order_relaxed);
  }
  // 返回待应用音量（0..200）并清空；无待应用返回 -1。仅编辑器 Timer（message 线程）调用。
  int consumePendingWebVolume() {
    return mPendingWebVolume.exchange(-1, std::memory_order_relaxed);
  }
