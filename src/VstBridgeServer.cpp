...
bool isAllowedOrigin(const std::string& origin) {
  if (origin.empty()) return true;  // 原生/桌面客户端（无 Origin 头）；浏览器无法伪造空 Origin
  ...
  if (const auto slash = s.find('/'); slash != std::string::npos) s = s.substr(0, slash);

  std::string host;
  if (!s.empty() && s.front() == '[') {                 // IPv6 字面量 [::1](:port)
  ...
  // 精确自定义域（prod + www + dev）。
  if (host == "synchain.cn" || host == "synchain.ca" ||
      host == "www.synchain.cn" || host == "www.synchain.ca" ||
      host == "dev.synchain.cn" || host == "dev.synchain.ca")
    return true;

  // Vercel 预览：synchain-git-<branch>-dl-snows-projects.vercel.app。
  // 要求 branch 段非空（host 长于前缀+后缀），排除前后缀重叠的边界串。
  const std::string pre = "synchain-git-";
  const std::string suf = "-dl-snows-projects.vercel.app";
...
void VstBridgeServer::pushPcm(const float* interleaved, int numSamples,
                              int sampleRate, int channels) noexcept {
  // 音频线程：noexcept、无锁、无分配、非阻塞。
  if (!mSenderRunning.load(std::memory_order_relaxed)) return;      // 无消费者
  if (mAudioClientCount.load(std::memory_order_relaxed) <= 0) return; // 无客户端
  if (interleaved == nullptr || channels <= 0 || numSamples <= 0) return;

  if (mPcmFifo.getFreeSpace() < 1) {   // ring 满：丢最新（SPSC 最简、无竞态、不阻塞）
  ...
  if (idx < 0 || idx >= static_cast<int>(mSlots.size())) { // 防御：slot 未就绪
...
void VstBridgeServer::pushMeter(float left, float right, float peak) noexcept {
  if (!mSenderRunning.load(std::memory_order_relaxed)) return;
  if (mAudioClientCount.load(std::memory_order_relaxed) <= 0) return;
...
void VstBridgeServer::stopSenderThread() {
  ...
  if (mSender.joinable()) mSender.join();
}
...
void VstBridgeServer::buildPcmFrame(const PcmSlot& slot) {
  ...
  const size_t dataSize =
      static_cast<size_t>(slot.numSamples) * static_cast<size_t>(slot.channels) * sizeof(float);
...
void VstBridgeServer::senderThreadRun() {
  while (mSenderRunning.load(std::memory_order_acquire)) {
    ...
    if (!mSenderRunning.load(std::memory_order_acquire)) break;
    ...
    for (;;) {
      ...
      {
        std::lock_guard<std::mutex> lock(mStreamMutex);
        if (mPcmFifo.getNumReady() < 1) break;
        int s1, bs1, s2, bs2;
      ...
      if (haveFrame) sendFrameToClients();
    }

    if (mMeterDirty.exchange(false, std::memory_order_acquire)) {
      broadcastMeter(mMeterL.load(std::memory_order_relaxed),
                     mMeterR.load(std::memory_order_relaxed),
...
void VstBridgeServer::stopHeartbeatThread() {
  ...
  if (mHeartbeat.joinable()) mHeartbeat.join();
}
...
void VstBridgeServer::heartbeatThreadRun() {
  while (mHeartbeatRunning.load(std::memory_order_acquire)) {
    ...
    if (!mHeartbeatRunning.load(std::memory_order_acquire)) break;
    if (!mServer) continue;
    ...
    int liveCount = 0;
    {
      ...
      for (auto it = mLastPongMs.begin(); it != mLastPongMs.end();) {
        ...
        for (const auto& c : clients)
          if (c.get() == it->first) { present = true; break; }
        if (!present) it = mLastPongMs.erase(it);
        else ++it;
      }
      ...
      for (const auto& c : clients) {
        ...
        if (now - it->second > kHeartbeatTimeoutMs) toClose.push_back(c);
        else                                        toPing.push_back(c);
      }
    }
    ...
    for (auto& c : toPing)  c->send(pingStd);
    for (auto& c : toClose) c->close(4408, "heartbeat timeout");
  }
}
