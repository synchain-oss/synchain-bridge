...
{
    std::function<void(const juce::String& projectId, const juce::String& userId, const juce::String& username)>
        onHandshake;
...
public:
    ...
    void sendStatus(bool connected, const juce::String& pluginName, const juce::String& version, int volumePct = 100);
    ...
    void pushPcm(const float* interleaved, int numSamples, int sampleRate, int channels) noexcept;
    void pushMeter(float left, float right, float peak) noexcept;
    uint64_t droppedPacketCount() const noexcept { return mDropped.load(std::memory_order_relaxed); }

    void sendMeterLevels(float left, float right, float peak);
    void sendSettings(int sampleRate, int bufferSize, int channels, int opusBitrate, double latencyMs = 0.0);
...
private:
    void onClientMessage(std::shared_ptr<ix::ConnectionState> connectionState, ix::WebSocket& client,
