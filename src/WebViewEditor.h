...
class SynchainBridgeWebEditor final : public juce::AudioProcessorEditor,
                                      private juce::Timer {
public:
...
private:
  ...
  std::optional<juce::WebBrowserComponent::Resource>
    provideResource(const juce::String& url) const;
  static const char* mimeForExtension(const juce::String& ext);

  // --- 原生函数处理（均在 message 线程被调用）---
  void handleRequestInitialState(const juce::Array<juce::var>&,
                                 juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleToggleRun(const juce::Array<juce::var>&,
                       juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleSetPort(const juce::Array<juce::var>&,
                     juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleSetMasterGain(const juce::Array<juce::var>&,
                           juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleSetLang(const juce::Array<juce::var>&,
                     juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleSetUiScale(const juce::Array<juce::var>&,
                        juce::WebBrowserComponent::NativeFunctionCompletion);
  void handleCommitUiScale(const juce::Array<juce::var>&,
                           juce::WebBrowserComponent::NativeFunctionCompletion);
  juce::var buildSnapshot() const;

  // --- WebView2 运行时探测 / 兜底面板 ---
  enum class FallbackReason { MissingRuntime, LoadTimeout };
