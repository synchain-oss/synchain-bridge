...
- **桥 #2**：`VstBridgeServer`(ixwebsocket) ↔ 浏览器 WS 客户端（`web-contract/lib/vst-bridge.ts`）。协议契约见 `BRIDGE_CONTRACT.md`。
...
不想自己编译可从 [GitHub Releases](../../releases) 下载 Windows x64 预编译版
...
DAW 视为全新插件，与旧 SonicBridge 不冲突。macOS 同理，`.vst3` 放 `~/Library/Audio/Plug-Ins/VST3/`。
...
见 `web-contract/`（可脱离 Next.js 独立验证桥 #2）：

```bash
cd web-contract && npm install
npm run verify        # 单命令契约冒烟测试
npm run mock          # 起 ws://localhost:9420 的 mock 插件（发二进制 PCM 帧）
```
然后用浏览器打开 `web/index.html`（经 `bridge.js` 的 WsPreview 后端直连 mock）即可预览插件 UI + 交互，无需编译/DAW。

## CI

`.github/workflows/vst3.yml`（windows-2022）：clone JUCE 8.0.8 → vcpkg 装 ixwebsocket → 装 WebView2 Evergreen Runtime → CMake 配置（拉 WebView2 NuGet）→ 构建 → pluginval `--skip-gui-tests`（strictness 5，验证协议/状态/参数/音频/总线等非 GUI 契约）。含 Editor 的全量 strictness-5 在真实 Windows 11 本地验证——无桌面的 Server runner 无法托管 WebView2 编辑器。仅在 PR 改动 `vst-plugin/**` 时触发。
...
| `web-contract/` | 桥 #2 web 客户端契约 + mock server（独立验证；见 MIGRATION.md） |
