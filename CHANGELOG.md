...
- **开窗第二段白:首帧信号改由「页面真的画过一帧」触发(SL-433,1.5.3)**:
  ...
  - ⚠ **SCVB 那边的真机数(13/13 正常、`signal − first-paint` 恒为正)不能外推到本仓** ——
    Bridge 是另一张页面、另一套资源。本机在真 WebView2 宿主(pluginval)上实测**修后**余量为
    `+14~18 ms`(8/8 为正),这证明**新路在 Bridge 上确实生效**;但它**回答不了**「改前那条信号
    是不是早于首帧」—— 那个量要在旧页面上测,而旧页面根本不带这个字段。**第二段白是否就此
    消失,仍要用户真机回验。**
  ...
  **22 格删除式反向注入逐格核过「红在设计接住它的那条断言上」**(不是只看红)。
