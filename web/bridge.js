...
const EVT = { STATE: "bridge.state", METER: "bridge.meter", AUDIO: "bridge.audio", ERROR: "bridge.error" };
...
function makeWsPreviewBackend(init, url) {
  const em = makeEmitter();
  const state = { running: false, clients: 0, port: init.port, volume: init.volume, lang: init.lang,
                  sampleRate: 48000, channels: 2, latencyMs: null };
  let ws = null;
  ...
  function connect() {
    ...
    ws.onopen = () => {
      state.running = true; state.clients = 1;
      ws.send(JSON.stringify({ type: "handshake", projectId: "preview", userId: "preview", username: "Preview" }));
      em.emit(EVT.STATE, { running: state.running, clients: state.clients, port: state.port });
    };
    ws.onmessage = (e) => {
      if (e.data instanceof ArrayBuffer) return; // 预览不解码 PCM
      let m; try { m = JSON.parse(e.data); } catch { return; }
      if (m.type === "meter") {
        em.emit(EVT.METER, { l: dbToLinear(m.left), r: dbToLinear(m.right), ldb: m.left, rdb: m.right, peak: m.peak });
      } else if (m.type === "settings") {
        state.sampleRate = m.sampleRate; state.channels = m.channels; state.latencyMs = m.latencyMs ?? null;
        em.emit(EVT.AUDIO, { sampleRate: m.sampleRate, channels: m.channels, latencyMs: state.latencyMs });
      } else if (m.type === "status") {
        state.running = true;
        em.emit(EVT.STATE, { running: true, clients: state.clients, port: state.port });
      } else if (m.type === "ping") {
        ws.send(JSON.stringify({ type: "pong", timestamp: m.timestamp }));
      }
    };
    ws.onclose = () => {
      state.running = false; state.clients = 0;
  ...
  return {
    ...
    async requestInitialState() {
      return { running: state.running, clients: state.clients, port: state.port,
               volume: state.volume, lang: state.lang, version: init.version, uiScale: init.uiScale,
               sampleRate: state.sampleRate, channels: state.channels, latencyMs: state.latencyMs };
    },
    async toggleRun(shouldRun) {
      if (shouldRun) { connect(); return { running: true, port: state.port }; }
      if (ws) ws.close();
      return { running: false, port: state.port };
    },
    async setPort(port) { state.port = port | 0; return { ok: true, port: state.port }; },
    async setMasterGain(pct) { state.volume = pct | 0; return { ok: true }; },
    async setLang(code) { state.lang = String(code); return { ok: true }; },
    // 预览无原生窗口：仅回执，缩放由 index.html 本地对卡片应用 zoom（applyPreviewFit）演示。
    async setUiScale(factor) { return { ok: true, scale: Number(factor) || 1 }; },
    // 预览无全局设置文件：确认「保持」的回执 no-op。
    async commitUiScale() { return { ok: true }; },
  };
}
